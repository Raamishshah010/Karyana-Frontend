...
const exportToPdf = (ledgerData, retailer, totals, dateFrom, dateTo) => {
  ...
  const drawPageHeader = (pageNum, totalPages) => {
    ...
    doc.setFontSize(8.5);
    doc.setTextColor(...DARK);
    doc.text(`Salesperson: ${retailer.salesperson || ""}`, pw / 2, 32, {
      align: "center",
    });
    ...
    doc.setFont("helvetica", "normal");
    doc.text(`Date From: ${fromLabel}  to: ${toLabel}`, pw - mr, 29, {
      align: "right",
    });
    doc.setFont("helvetica", "bold");
    doc.text(`Total Due :Rs. ${formatNumber(totals.finalBal)}`, pw - mr, 34, {
      align: "right",
    });
    ...
    if (pageNum > 1) {
      ...
      doc.text(`Page ${pageNum} of ${totalPages}`, pw - mr, 52, {
        align: "right",
      });
    }
  ...
  const drawTableHeader = (y) => {
    ...
    cols.forEach((col) => {
      const xPos = ml + col.x;
      if (col.align === "right")
        doc.text(col.label, xPos + col.w - 1, y + 5, { align: "right" });
      else if (col.align === "center")
        doc.text(col.label, xPos + col.w / 2, y + 5, { align: "center" });
  ...
  const totalPages =
    1 + (remainingRows > 0 ? Math.ceil(remainingRows / rowsPerOtherPage) : 0);
  ...
  if (retailer.openingBalance != null)
    doc.text(
      formatNumber(retailer.openingBalance),
      ml + cols[8].x + cols[8].w - 1,
      y + 5,
      { align: "right" },
    );
  doc.setDrawColor(...LGRAY);
  ...
  ledgerData.forEach((row) => {
    ...
    [
      { col: 0, v: row.date },
      {
        col: 1,
        v:
          row.details.length > 18
            ? row.details.slice(0, 17) + "…"
            : row.details,
      },
      ...
      const c = cols[col],
        xPos = ml + c.x;
      if (c.align === "right")
        doc.text(v, xPos + c.w - 1, y + 5, { align: "right" });
      else if (c.align === "center")
        doc.text(v, xPos + c.w / 2, y + 5, { align: "center" });
  ...
  doc.setFontSize(7.5);
  doc.setTextColor(...BLACK);
  const totalQty = ledgerData.reduce(
    (s, r) => s + (Number(r.quantity) || 0),
    0,
  );
  doc.text(String(totalQty), ml + cols[5].x + cols[5].w - 1, y + 5, {
    align: "right",
  });
  doc.text(
    formatNumber(totals.totalDr),
    ml + cols[6].x + cols[6].w - 1,
    y + 5,
    { align: "right" },
  );
  doc.text(
    formatNumber(totals.totalCr),
    ml + cols[7].x + cols[7].w - 1,
    y + 5,
    { align: "right" },
  );
  doc.line(ml, y + 7, ml + cw, y + 7);
  ...
  doc.save(
    `Ledger_${(retailer.name || "customer").replace(/\s+/g, "_")}_${new Date().toISOString().slice(0, 10)}.pdf`,
  );
...
const exportToExcel = (ledgerData, retailer, totals, dateFrom, dateTo) => {
  ...
  rows.push([
    ...
    "",
    "Phone:",
    retailer.phoneNumber || retailer.phone || "",
  ]);
  rows.push([
    "Date From:",
    dateFrom ? isoToDisplay(dateFrom) : "All",
    "to:",
    dateTo ? isoToDisplay(dateTo) : new Date().toLocaleDateString("en-GB"),
    "",
    "Total Due:",
    totals.finalBal,
  ]);
  ...
  rows.push([
    ...
    totals.totalDr,
    totals.totalCr,
    ledgerData[ledgerData.length - 1]?.rawBalance || 0,
  ]);
  ...
  XLSX.writeFile(
    wb,
    `Ledger_${(retailer.name || "customer").replace(/\s+/g, "_")}_${new Date().toISOString().slice(0, 10)}.xlsx`,
  );
...
const ReportsCustomerLedger = () => {
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      ...
      if (rawArray.length === 0) {
        ...
        const normalised = rawArray.map((item, idx) =>
          normaliseEntry(item, idx),
        );
  ...
  const totals = {
    ...
    finalBal: ledgerData.length
      ? ledgerData[ledgerData.length - 1].rawBalance
      : 0,
  };
            ...
            <div className="bg-white border border-gray-100 rounded-2xl shadow-sm p-6">
              ...
              <div className="mb-5">
                ...
                <div className="relative">
                  ...
                  >
                    {selectedRetailer ? (
                      <div className="flex items-center gap-2 flex-1 min-w-0">
                        <div className="w-7 h-7 rounded-full bg-[#FF5934]/10 flex items-center justify-center flex-shrink-0">
                          <span className="text-[#FF5934] text-[11px] font-bold">
                            {(selectedRetailer.name || "?")[0].toUpperCase()}
                          </span>
                        </div>
                    ...
                    <MdExpandMore
                      size={18}
                      className={`text-[#9CA3AF] flex-shrink-0 transition-transform ${dropdownOpen ? "rotate-180 text-[#FF5934]" : ""}`}
                    />
                  </div>

                  {dropdownOpen && (
                    <div
                      className="absolute z-30 top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-2xl shadow-2xl overflow-hidden rcl-no-scroll"
                        ...
                        filteredRetailers.map((r) => (
                          ...
                          >
                            ...
                            <div className="min-w-0 flex-1">
                              ...
                              <p className="text-[11px] text-[#9CA3AF] truncate">
                                {r.shopName || r.shopCategory || ""} ·{" "}
                                {r.phoneNumber || r.phone || "—"}
                              </p>
                            </div>
                            {selectedRetailer?._id === r._id && (
                              <MdCheckCircle
                                size={15}
                                className="text-[#FF5934] flex-shrink-0"
                              />
                            )}
                          </div>
                        ))
                ...
                <div className="mb-5 p-3 bg-[#F9FAFB] rounded-xl border border-gray-100 flex flex-wrap gap-4 text-[12px] text-[#6B7280]">
                  {selectedRetailer.shopName && (
                    <span className="flex items-center gap-1.5">
                      <MdStorefront size={13} className="text-[#FF5934]" />
                      {selectedRetailer.shopName}
                    </span>
                  )}
                  {(selectedRetailer.phoneNumber || selectedRetailer.phone) && (
                    <span className="flex items-center gap-1.5">
                      <MdPhone size={13} className="text-[#FF5934]" />
                      {selectedRetailer.phoneNumber || selectedRetailer.phone}
                    </span>
                  )}
              ...
              <div className="mb-6">
                <label className="flex items-center gap-1.5 text-[11px] font-bold text-[#6B7280] uppercase tracking-widest mb-2">
                  ...
                  Range
                  <span className="text-[#9CA3AF] font-normal normal-case tracking-normal ml-1">
                    (optional — leave blank to show all)
                  </span>
                </label>
                <div className="flex items-center gap-3">
                  <div className="flex-1">
                    <p className="text-[10px] text-[#9CA3AF] mb-1.5 font-semibold uppercase tracking-wide">
                      From
                    </p>
                    <input
                      type="date"
                  ...
                  <span className="mt-5 text-[#9CA3AF] text-sm font-medium flex-shrink-0">
                    →
                  </span>
                  <div className="flex-1">
                    <p className="text-[10px] text-[#9CA3AF] mb-1.5 font-semibold uppercase tracking-wide">
          ...
          <div className="fade-up">
                ...
                <p className="text-sm text-[#9CA3AF] mt-0.5">
                  ...
                  {allLedgerData.length > 0 && (
                    <span className="ml-2 text-[#FF5934] font-semibold">
                      {allLedgerData.length} total transaction
                      {allLedgerData.length !== 1 ? "s" : ""}
                    </span>
                  )}
                </p>
              </div>
            ...
            <div className="flex flex-wrap items-center gap-2 mb-5 bg-white border border-gray-100 rounded-2xl shadow-sm px-4 py-3">
              <MdCalendarToday
                ...
                className="text-[#FF5934] flex-shrink-0"
              />
              <span className="text-[11px] font-bold text-[#6B7280] uppercase tracking-widest mr-1">
                Filter by Date:
              </span>
              <input
                type="date"
              ...
              {isDateFiltered ? (
                ...
                <span className="text-[11px] text-[#9CA3AF] ml-1">
                  Showing all {allLedgerData.length} transaction
                  {allLedgerData.length !== 1 ? "s" : ""}
                </span>
              )}
            </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-5">
                  {[
                    ...
                    },
                    {
                      icon: MdTrendingUp,
                      label: "Total Debit",
                      value: `Rs. ${formatNumber(totals.totalDr)}`,
                    ...
                    },
                    {
                      icon: MdTrendingDown,
                      label: "Total Credit",
                      value: `Rs. ${formatNumber(totals.totalCr)}`,
                      color: "text-red-500",
                      bg: "bg-red-50",
                    },
                    {
                      ...
                      label: "Balance",
                      value: `Rs. ${formatNumber(totals.finalBal)}`,
                      color:
                        totals.finalBal >= 0
                          ? "text-[#FF5934]"
                          : "text-red-600",
                    ...
                    <div
                      key={label}
                      className="bg-white border border-gray-100 rounded-2xl shadow-sm px-5 py-4 flex items-center gap-4 hover:-translate-y-0.5 transition-transform"
                    >
                      <div
                      ...
                      <div className="min-w-0">
                        <p className="text-[11px] font-bold text-[#9CA3AF] uppercase tracking-widest mb-0.5">
                          {label}
                        </p>
                        <p
                          className={`text-[14px] font-bold truncate ${color}`}
                        >
                          {value}
                        </p>
                      </div>
                ...
                <div className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
                      ...
                      <thead>
                        <tr className="bg-[#FFF4F1] border-b border-orange-100">
                          {[
                            ...
                            <th
                              key={h.label}
                              className={`text-[11px] font-bold text-[#9CA3AF] uppercase tracking-widest py-3 ${h.cls}`}
                            >
                              {h.label}
                            </th>
                          ))}
                        ...
                        {ledgerData.map((row) => (
                          <tr key={row.id || row.sr} className="trow">
                            ...
                            <td className="px-3 py-2.5 max-w-[160px]">
                              ...
                              <span
                                className={`inline-flex items-center px-1.5 py-0.5 rounded text-[9px] font-bold mt-0.5 ${
                                  row.type === "PURCHASE"
                            ...
                            <td className="px-3 py-2.5 max-w-[140px]">
                              <span className="text-[12px] text-[#6B7280] break-words">
                                {row.refNo || "—"}
                              </span>
                            </td>
                            <td className="px-3 py-2.5 text-center">
                  ...
                  <div className="border-t-2 border-orange-100 bg-[#FFF4F1] px-4 py-3 flex flex-wrap items-center justify-between gap-4">
                    <p className="text-[11px] font-bold text-[#9CA3AF] uppercase tracking-widest">
                      {ledgerData.length} of {allLedgerData.length} transaction
                      {allLedgerData.length !== 1 ? "s" : ""}
                      {isDateFiltered && (
                        <span className="ml-1.5 text-[#FF5934]">
                          (date filtered)
                        </span>
                      )}
                    </p>
                    <div className="flex items-center gap-6 flex-wrap">
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-[14px] font-bold text-red-500">
                          Rs. {formatNumber(totals.totalCr)}
                        </p>
                      </div>
                      <div className="w-px h-8 bg-gray-200" />
                      <div className="text-right">
                        ...
                        <p className="text-[14px] font-bold text-emerald-600">
                          Rs. {formatNumber(totals.totalDr)}
                        </p>
                      </div>
                      ...
                      <div className="text-right">
                        ...
                        <p
                          className={`text-[14px] font-bold ${totals.finalBal >= 0 ? "text-[#FF5934]" : "text-red-600"}`}
                        >
            ...
            {!loading && ledgerData.length === 0 && (
              <div className="bg-white border border-gray-100 rounded-2xl shadow-sm py-20 text-center flex flex-col items-center gap-3">
                ...
                {isDateFiltered ? (
                  <button
                    onClick={handleClearDates}
                    className="text-[#FF5934] text-xs hover:underline font-semibold"
                  >
