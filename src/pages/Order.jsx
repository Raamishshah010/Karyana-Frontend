...
const generateOrderPDF = (item) => {
 ...
 const rows = (item.items || []).map(oi => {
    const isCtn   = oi.type === 'ctn';
    const isPcs   = oi.type === 'piece' || oi.type === 'pcs' || (!isCtn && oi.type !== 'ctn');
    const ctnQty  = isCtn   ? oi.quantity : '—';
    const pcsQty  = !isCtn  ? oi.quantity : '—';
    return `
    <tr>
      <td style="padding:8px 12px;border-bottom:1px solid #f3f4f6;font-size:12px;color:#374151">${oi.productId?.englishTitle || '—'}</td>
      <td style="padding:8px 12px;border-bottom:1px solid #f3f4f6;font-size:12px;color:#374151;text-align:center">${ctnQty}</td>
      <td style="padding:8px 12px;border-bottom:1px solid #f3f4f6;font-size:12px;color:#374151;text-align:center">${pcsQty}</td>
      ...
      <div style="border:1px solid #f3f4f6;border-radius:12px;overflow:hidden;margin-bottom:24px">
        ...
        <table style="width:100%;border-collapse:collapse">
          <thead><tr style="background:#fafafa">
            ...
            <!-- NEW -->
<th style="padding:8px 12px;text-align:center;font-size:10px;font-weight:700;color:#9ca3af;text-transform:uppercase">Qty (CTNs)</th>
<th style="padding:8px 12px;text-align:center;font-size:10px;font-weight:700;color:#9ca3af;text-transform:uppercase">Qty (PCS)</th>
