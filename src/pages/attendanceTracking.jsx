...
import { getAttendanceBySalesId, getAllSalesPersons, getAllCities } from "../APIS";
import { Loader } from "../components/common/loader";
import {
  MdSearch, MdClose, MdRefresh, MdPerson, MdCalendarToday,
  MdAccessTime, MdBarChart, MdCheckCircle, MdLogout, MdLogin,
  MdTimer, MdFilterList, MdPictureAsPdf, MdGridOn, MdLocationOn,
  MdExpandMore,
} from "react-icons/md";
...
const reportHeaders = [
  ...
  "Total Hours Worked", "Status", "Remarks",
];

const safeFilePart = (value) =>
  String(value || "attendance-report").trim()
...
const getDateRangeLabel = (rows, startDate, endDate) => {
  ...
  const validDates = rows
    .map((row) => row.Date).filter((d) => d && d.includes("/"))
    .sort((a, b) => {
...
/* ── resolve city ID from a salesperson's city field ── */
const resolveCityId = (sp) => {
  const c = sp?.city || sp?.cityID;
  if (!c) return null;
  return typeof c === "object" ? (c._id || null) : c;
};
...
const AttandanceTracking = () => {
  ...
  /* ── load salespersons + cities on mount ── */
  useEffect(() => {
    Promise.all([getAllSalesPersons(), getAllCities()])
      .then(([spRes, cityRes]) => {
  ...
  /* ─────────────────────────────────────────────────────────
     DERIVED: salespersons filtered by selected city
     When no city is selected → show all salespersons
     When a city is selected  → only show salespersons whose
       city field matches the selected city _id
  ───────────────────────────────────────────────────────── */
  const cityFilteredSPs = selectedCity
    ? salesPersons.filter((sp) => resolveCityId(sp) === selectedCity)
    : salesPersons;

  /* apply text search on top of city filter */
  const filteredSP = cityFilteredSPs.filter((sp) =>
    (sp.name || "").toLowerCase().includes(spSearch.toLowerCase())
  );

  /* city list filtered by search */
  ...
  const handleReset = () => {
    ...
    setSelectedSalesPerson(""); setSelectedCity("");
    setAttendanceData([]); setGenerated(false);
    setSpSearch(""); setCitySearch("");
  };
  ...
  const handleExportPdf = () => {
    ...
    const margin = 10, footerHeight = 12, rowMinHeight = 8;
    const columns = [
    ...
    reportRows.forEach((row) => {
      ...
      const rowHeight = Math.max(rowMinHeight, ...cellLines.map((l) => l.length * 4 + 4));
      if (y + rowHeight > pageHeight - footerHeight) {
        doc.addPage(); y = drawReportHeader();
        doc.setFont("helvetica", "normal"); doc.setFontSize(7.5);
      }
      let x = margin;
        ...
        <div className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-visible mb-0">
          ...
          <div className="px-5 py-5 border-b border-gray-100 bg-[#FAFAFA]">
            ...
            <div className="flex flex-wrap gap-4 items-end">
                  ...
                  <div className="absolute z-30 top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-2xl shadow-2xl overflow-hidden">
                    ...
                    <div className="p-2 border-b border-gray-100">
                      <div className="flex items-center gap-2 bg-[#F9FAFB] border border-gray-200 rounded-xl px-3 py-1.5">
                        ...
                        {citySearch && <button onClick={() => setCitySearch("")} className="text-[#9CA3AF] hover:text-[#FF5934]"><MdClose size={13} /></button>}
                      </div>
                    </div>
                    <div className="max-h-52 overflow-y-auto att-no-scroll">
                      ...
                      <div
                        onClick={() => {
                          setSelectedCity("");
                          setSelectedSalesPerson(""); // reset SP when location cleared
                          setAttendanceData([]); setGenerated(false);
                          setCityDropOpen(false); setCitySearch("");
                        }}
                      ...
                      {filteredCities.length > 0 ? filteredCities.map((city) => (
                        <div key={city._id}
                          onClick={() => {
                            ...
                            setSelectedSalesPerson(""); // reset SP when location changes
                            setAttendanceData([]); setGenerated(false);
                            setCityDropOpen(false); setCitySearch("");
                          }}
                          className={`flex items-center gap-3 px-3 py-2.5 cursor-pointer hover:bg-orange-50 transition-colors border-b border-gray-50 last:border-0
                            ${selectedCity === city._id ? "bg-orange-50" : ""}`}
                        >
                          <div className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0
                            ${selectedCity === city._id ? "bg-[#FF5934]/10" : "bg-gray-100"}`}>
                            <MdLocationOn size={14} className={selectedCity === city._id ? "text-[#FF5934]" : "text-gray-400"} />
                          </div>
                          <p className={`text-[13px] flex-1 truncate ${selectedCity === city._id ? "font-semibold text-[#111827]" : "font-medium text-[#374151]"}`}>
                            {city.name}
                          </p>
              ...
              {/* ── Sales Person Picker ── */}
              <div className="flex-1 min-w-[220px] att-dropdown">
                <label className="flex items-center gap-1.5 text-[11px] font-bold text-[#6B7280] uppercase tracking-widest mb-1.5">
                  <MdPerson size={12} className="text-[#FF5934]" />
                  Sales Person
                  {/* badge: shows how many SPs are in the selected location */}
                  {selectedCity && (
                    <span className="ml-1 text-[10px] font-semibold text-violet-500 bg-violet-50 border border-violet-100 px-1.5 py-0.5 rounded-full normal-case tracking-normal">
                      {cityFilteredSPs.length} in location
                    </span>
                  )}
                </label>
                <div
                  className={`flex items-center gap-2 bg-white border rounded-xl px-3 py-2.5 cursor-pointer transition-all
                    ${spDropOpen ? "border-[#FF5934] shadow-sm shadow-orange-100" : "border-gray-200 hover:border-[#FF5934]"}`}
                ...
                >
                  {selectedPersonObj ? (
                    ...
                    <span className="text-[13px] text-gray-300 flex-1">
                      {selectedCity ? `${cityFilteredSPs.length} salesperson${cityFilteredSPs.length !== 1 ? "s" : ""} in location…` : "Select sales person…"}
                    </span>
                  )}
                ...
                {spDropOpen && (
                  <div className="absolute z-30 top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-2xl shadow-2xl overflow-hidden">
                    ...
                    {/* Context hint when location is selected */}
                    {selectedCity && (
                      <div className="px-3 py-2 bg-violet-50 border-b border-violet-100">
                        <p className="text-[11px] text-violet-600 font-semibold flex items-center gap-1.5">
                          ...
                          Showing {cityFilteredSPs.length} salesperson{cityFilteredSPs.length !== 1 ? "s" : ""} in {selectedCityObj?.name}
                        </p>
                      </div>
                    )}
                    <div className="max-h-56 overflow-y-auto att-no-scroll">
                      {filteredSP.length ? filteredSP.map(sp => (
                        <div key={sp._id}
                          onClick={() => { setSelectedSalesPerson(sp._id); setSpDropOpen(false); setSpSearch(""); }}
                          className={`flex items-center gap-3 px-3 py-2.5 cursor-pointer hover:bg-orange-50 transition-colors border-b border-gray-50 last:border-0
                            ${selectedSalesPerson === sp._id ? "bg-orange-50" : ""}`}
                        >
                        ...
                        <div className="py-8 text-center flex flex-col items-center gap-2">
                          ...
                          <p className="text-[13px] text-[#9CA3AF]">
                            {selectedCity ? `No salespersons found in ${selectedCityObj?.name}` : "No results found"}
                          </p>
                        </div>
                      )}
                    </div>
              ...
              <div className="flex-1 min-w-[160px]">
                ...
                <input type="date" value={endDate} onChange={e => setEndDate(e.target.value)}
                  min={startDate || undefined} className="att-input" />
              </div>
            ...
            {/* Active filter chips */}
            {(selectedPersonObj || selectedCityObj || startDate || endDate) && (
              <div className="mt-4 flex flex-wrap items-center gap-2">
                ...
                {selectedCityObj && (
                  <div className="flex items-center gap-2 bg-violet-50 border border-violet-100 rounded-xl px-3 py-1.5">
                    ...
                    <span className="text-[12px] font-semibold text-violet-600">{selectedCityObj.name}</span>
                    <button
                      onClick={() => { setSelectedCity(""); setSelectedSalesPerson(""); setAttendanceData([]); setGenerated(false); }}
                ...
                {selectedPersonObj && (
                  <div className="flex items-center gap-2 bg-orange-50 border border-orange-100 rounded-xl px-3 py-1.5">
                    ...
                    <button
                      onClick={() => { setSelectedSalesPerson(""); setAttendanceData([]); setGenerated(false); }}
          ...
          {loading ? (
            ...
            <div className="py-20 text-center flex flex-col items-center gap-3">
              ...
              <p className="text-[#9CA3AF] text-sm font-medium">
                {selectedCity && !selectedSalesPerson
                  ? `${cityFilteredSPs.length} salesperson${cityFilteredSPs.length !== 1 ? "s" : ""} in ${selectedCityObj?.name} — select one to generate`
                  : "Select a sales person and click Generate Report"}
              </p>
            </div>
