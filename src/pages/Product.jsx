...
const Product = () => {
  ...
  const filteredFilterBrands = selectedCategoryId
    ? brands.data.filter(
        (brand) => getBrandCategoryId(brand) === selectedCategoryId,
      )
    : brands.data;
  ...
  const memoizedGetBrandName = useCallback(
    (brand) => getBrandName(brand),
    [brands.data],
  );
  const memoizedGetCategoryName = useCallback(
  ...
  useEffect(() => {
    if (
      isCoordinator &&
      coordinatorCityId &&
      selectedCityId !== coordinatorCityId
    ) {
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      toast.error(
        error?.response?.data?.msg || "Failed to delete selected products",
      );
  ...
  const validations = yup.object().shape({
    ...
    price: yup
      ...
      .test(
        "price-test",
        "Sales rate must be >= purchase rate",
        function (value) {
          const { purchaseRate } = this.parent;
          return (
            (value === 0 && purchaseRate === 0) ||
            (value !== undefined &&
    ...
    purchaseDiscount: yup
      ...
      .transform((v) => (isNaN(v) ? 0 : v))
      .min(0, "Discount can't be negative"),
    purchaseTradeOffer: yup
      .number()
      .transform((v) => (isNaN(v) ? 0 : v))
      .min(0, "Trade offer can't be negative"),
    saleTradeOffer: yup
      .number()
      .transform((v) => (isNaN(v) ? 0 : v))
      .min(0, "Trade offer can't be negative"),
  ...
  const changeHandler = async (key, value) => {
    if (key === "cityID") {
      ...
      try {
        ...
        setState((p) => ({ ...p, [key]: value, brandID: "" }));
      } catch (error) {
  ...
  const handleSubmit = async (values) => {
    ...
    if (
      ...
      Number(values.purchaseDiscount) > 100
    ) {
    ...
    try {
      ...
      const updateData = {
        ...
        isDiscounted: Number(values.discount || 0) > 0,
      ...
      clearForm();
      toast.success(
        `Product ${state.id ? "updated" : "created"} successfully!`,
      );
    } catch (error) {
      toast.error(
  ...
  const updateDataHandler = async (checked, name, item) => {
    try {
      ...
      const updatedData = data.map((product) =>
        product._id === item._id ? { ...product, [name]: checked } : product,
      );
      setData(updatedData);
    ...
    try {
      ...
      if (categoryId) {
        try {
          ...
          const brandExists = brandsForCategory.some(
            (brand) =>
              brand._id === brandId ||
      ...
      if (brandId && brandsToLoad.length > 0) {
        const brandExists = brandsToLoad.some(
          (brand) =>
            brand._id === brandId ||
            (brand.brandId && brand.brandId === brandId),
        );
        if (!brandExists) {
          try {
            ...
            if (brandRes.data.data) brandsToLoad.push(brandRes.data.data);
          } catch {}
        }
      }
      ...
      setState({
        ...
        purchaseDiscount2: item.purchaseDiscount || 0,
  ...
  const addHandler = async () => {
    clearForm();
    // Load all categories and brands upfront — no site dependency
    try {
      const [catRes, brandRes] = await Promise.all([
        getAllCategories(),
        getAllBrands(),
      ]);
      setFormCategories({ isLoaded: true, data: catRes.data.data });
      setFormBrands({ isLoaded: true, data: brandRes.data.data });
    } catch {
  ...
  const saveBulkDiscount = async () => {
    const validRows = discountRows.filter(
      (row) => row.quantity && row.discount !== "" && row.type,
    );
    ...
    try {
      setLoading(true);
      const bulkDiscountQuantity = validRows.map((row) =>
      ...
      const updateData = {
        ...
        categoryID:
          bulkDiscountProduct.category?._id || bulkDiscountProduct.category,
      ...
      setData(
        data.map((product) =>
          product._id === bulkDiscountProduct._id
            ? {
                ...product,
                bulkDiscountQuantity,
                bulkDiscount,
  ...
  const handleExcelImport = async (e) => {
    ...
    try {
      ...
      const codeIndex = headers.findIndex(
        (h) => h && h.toString().trim().toLowerCase() === "code",
      );
      const locationCodeIndex = headers.findIndex(
        (h) => h && h.toString().trim().toLowerCase() === "fieldb",
      );
      if (codeIndex === -1 || locationCodeIndex === -1)
        throw new Error("Required columns not found");
      const filteredRows = dataRows.filter((row) => {
        const pc = row[codeIndex];
        return pc && String(pc).trim() !== "";
      });
      ...
      filteredRows.forEach((row) => {
        ...
        const matchingCity = citiesData.find(
          (city) => String(city.locationId).trim() === locationCode.trim(),
        );
        if (!matchingCity) {
        ...
        const productData = {
          ...
          category: (() => {
            ...
            if (!cn) return null;
            const mc = allCategories.find(
              (cat) => cat.englishName?.toLowerCase() === cn.toLowerCase(),
            );
            if (!mc) {
              skippedProducts.push(
                `Skipping ${productCode}: Category "${cn}" not found`,
              );
              return null;
            }
            return mc._id;
          ...
          brand: (() => {
            ...
            if (!bn) {
              skippedProducts.push(
                `Skipping ${productCode}: No brand specified`,
              );
              return null;
            }
            const mb = allBrands.find(
              (brand) => brand.englishName?.toLowerCase() === bn.toLowerCase(),
            );
            if (!mb) {
              skippedProducts.push(
                `Skipping ${productCode}: Brand "${bn}" not found`,
              );
              return null;
            }
            return mb._id;
      ...
      if (jsonData.length === 0) {
        let em = "No valid products to import.";
        if (skippedProducts.length > 0)
          em += "\n\nSkipped:\n" + skippedProducts.join("\n");
        throw new Error(em);
      } else if (skippedProducts.length > 0)
        toast.warn(`${skippedProducts.length} products were skipped.`);
      const BATCH_SIZE = 50;
      let totalImported = 0,
        totalUpdated = 0,
        totalFailed = 0;
      ...
      for (let i = 0; i < jsonData.length; i += BATCH_SIZE) {
        ...
        try {
          const res = await fetch(
            "https://primelinkdistribution.com/api/product/import-excel",
            {
              method: "POST",
              headers: {
          ...
          if (result.msg === "success") {
            ...
            throw new Error(
              result.message || "Unknown error during batch import",
            );
      ...
      if (totalFailed > 0) {
        ...
        if (allErrors.length > 0)
          toast.error(`Some items failed. Check console for details.`);
  ...
  const handleExportToExcel = () => {
    try {
      ...
      const exportData = products.map((product) => ({
        ...
        "Sale Price/Rate": product.saleRate
          ? Math.floor(product.saleRate).toString()
          : product.price
            ? Math.floor(product.price).toString()
            : "0",
        Cost: product.purchaseRate
          ? Math.floor(product.purchaseRate).toString()
          : "0",
      ...
      <div className="prd-page">
        ...
        <div className="flex items-center justify-between mt-6 mb-5">
          <div>
            ...
            <p className="text-sm text-[#9CA3AF] mt-0.5">
              {data.length} products on this page
            </p>
          </div>
          <div className="flex items-center gap-2 flex-wrap justify-end">
            {selectedProductIds.length > 0 && (
              <button
                ...
                className="action-toolbar-btn danger"
              >
            ...
            {!role.includes(ROLES[2]) && (
              <button
                onClick={addHandler}
                className="action-toolbar-btn primary"
              >
                  ...
                  <tr key={index} className="table-row cursor-pointer">
                    ...
                    <td className="px-4 py-3">
                      <span className="text-[12px] text-[#374151] bg-[#F3F4F6] px-2 py-1 rounded-lg font-medium">
                        {categories.isLoaded
                          ? memoizedGetCategoryName(product.category)
                          : "…"}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <span className="text-[12px] text-[#374151] bg-[#F3F4F6] px-2 py-1 rounded-lg font-medium">
                        {brands.isLoaded
                          ? memoizedGetBrandName(product.brand)
                          : "…"}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      ...
                      <p className="text-[11px] text-[#9CA3AF]">
                        PR: {Math.floor(product.purchaseRate)}
                      </p>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex flex-wrap gap-1 max-w-[140px]">
                        {product.isPurchaseDiscounted &&
                          Number(product.purchaseDiscount) > 0 && (
                            <span className="inline-flex items-center px-1.5 py-0.5 rounded-md text-[10px] font-bold bg-purple-50 text-purple-600 ring-1 ring-purple-200 whitespace-nowrap">
                              PD {product.purchaseDiscount}
                              {product.purchaseDiscountType === "Flat"
                                ? " Rs"
                                : "%"}
                            </span>
                          )}
                        ...
                        {!product.isPurchaseDiscounted &&
                          !product.isDiscounted &&
                          !Number(product.purchaseTradeOffer) &&
                          !Number(product.saleTradeOffer) && (
                        ...
                        {!role.includes(ROLES[2]) && (
                          <>
                            ...
                            <button
                              onClick={() => {
                                const hasBulk =
                                  product.bulkDiscountQuantity?.length > 0 &&
                                  product.bulkDiscount?.length > 0;
                                setBulkDiscountProduct(product);
                                setDiscountRows(
                                  hasBulk
                                    ? product.bulkDiscountQuantity.map(
                                        (qty, i) => ({
                ...
                <tr>
                  <td colSpan={12} className="py-16 text-center">
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <button
                        onClick={refreshData}
                        className="text-[#FF5934] text-xs hover:underline"
                      >
                        Clear filters
                      </button>
                    </div>
                          ...
                          <div className="flex-1 overflow-hidden">
                            ...
                            {fileUpload ? (
                              ...
                              <div className="drag-drop-wrapper">
                                <DragNdrop
                                  onFilesSelected={fileUploadHandler}
                                  width="100%"
                                  height="36px"
                                />
                              </div>
                    ...
                    <div className="px-6 pb-6 flex flex-col gap-4">
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          ...
                          <FieldGroup icon={MdLocalOffer} label="Sale Discount">
                            <div className="grid grid-cols-[92px_1fr] gap-2">
                              <select
                                ...
                                onChange={(e) =>
                                  changeHandler("discountType", e.target.value)
                                }
                                className={`${inputCls} filter-select`}
                              >
                                <option value="Percentage">%</option>
                          ...
                          <FieldGroup
                            icon={MdLocalOffer}
                            label="Purchase Trade Offer (TO)"
                          >
                            <Input
                              name="purchaseTradeOffer"
                              placeholder="0"
                              type="number"
                              changeHandler={changeHandler}
                              className={inputCls}
        ...
        {selectedProduct && !stockModal && (
          <ClickOutside onClick={() => setSelectedProduct(null)}>
            ...
            >
              <div
                className="modal-card bg-white w-full max-w-[420px] rounded-3xl shadow-2xl overflow-hidden flex flex-col"
                onClick={(e) => e.stopPropagation()}
              >
                {/* Hero */}
                <div className="relative bg-gradient-to-br from-[#1a1a2e] via-[#16213e] to-[#0f3460] px-6 pt-6 pb-16 overflow-hidden">
                  ...
                  <div className="relative flex items-end gap-4">
                    ...
                    <div className="pb-1 min-w-0">
                      <h3 className="text-white text-[16px] font-bold leading-tight truncate">
                      ...
                      <div className="mt-2">
                        ...
                        >
                          <span
                            className={`w-1.5 h-1.5 rounded-full ${selectedProduct.isActive ? "bg-emerald-400" : "bg-gray-400"}`}
                          />
                ...
                <div className="-mt-6 mx-5 grid grid-cols-3 gap-2 z-10 relative">
                  {[
                    {
                      label: "Sale Price",
                      value: `Rs. ${Math.floor(selectedProduct.price)}`,
                    },
                    {
                      label: "Purchase",
                      value: `Rs. ${Math.floor(selectedProduct.purchaseRate)}`,
                    },
                    { label: "Stock", value: selectedProduct.stock },
                  ].map(({ label, value }) => (
                    <div
                      key={label}
                      className="bg-white rounded-2xl border border-gray-100 shadow-md px-3 py-3 text-center"
                    >
                      <p className="text-[14px] font-bold text-[#FF5934]">
                        {value}
                      </p>
                      <p className="text-[10px] text-[#9CA3AF] font-semibold uppercase tracking-wide mt-0.5">
                        {label}
                      </p>
                    </div>
                  ))}
                  ...
                  {[
                    {
                      icon: MdCategory,
                      label: "Category",
                      value: categories.isLoaded
                        ? memoizedGetCategoryName(selectedProduct.category)
                        : "…",
                    },
                    {
                      icon: MdBrandingWatermark,
                      label: "Brand",
                      value: brands.isLoaded
                        ? memoizedGetBrandName(
                            selectedProduct.brand || selectedProduct.brandID,
                          )
                        : "…",
                    },
                    {
                      icon: MdWarehouse,
                      label: "Carton Size",
                      value: selectedProduct.cortanSize,
                    },
                      ...
                      ? [
                          {
                            icon: MdLocalOffer,
                            label: "Sale Discount",
                            value: `${selectedProduct.discount}${selectedProduct.discountType === "Flat" ? " Rs." : "%"} off`,
                          },
                        ]
                      : []),
                    ...(selectedProduct.isPurchaseDiscounted
                      ? [
                          {
                            icon: MdLocalOffer,
                            label: "Purchase Discount",
                            value: `${selectedProduct.purchaseDiscount}${selectedProduct.purchaseDiscountType === "Flat" ? " Rs." : "%"} off`,
                          },
                        ]
                      : []),
                    ...(selectedProduct.purchaseTradeOffer
                      ? [
                          {
                            icon: MdLocalOffer,
                            label: "Purchase Trade Offer",
                            value: `${selectedProduct.purchaseTradeOffer} TO`,
                          },
                        ]
                      : []),
                    ...(selectedProduct.saleTradeOffer
                      ? [
                          {
                            icon: MdLocalOffer,
                            label: "Sale Trade Offer",
                            value: `${selectedProduct.saleTradeOffer} TO`,
                          },
                        ]
                      : []),
                    ...
                    <div
                      key={label}
                      className="flex items-start gap-3 bg-[#F9FAFB] rounded-2xl px-4 py-3 border border-gray-100"
                    >
                    ...
                    <div className="bg-[#F9FAFB] rounded-2xl px-4 py-3 border border-gray-100">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {selectedProduct.bulkOrders.map((it, i) => (
                          <div
                            key={i}
                            className="bg-white border border-gray-200 rounded-xl px-3 py-2 text-center shadow-sm"
                          >
                            <p className="text-[11px] text-[#9CA3AF]">{`> ${it.quantity} items`}</p>
                            <p className="text-[13px] font-bold text-[#FF5934]">
                              {Math.floor(it.amount)} Rs
                            </p>
                          </div>
                        ))}
                      </div>
                ...
                <div className="px-5 pb-5 pt-2 flex gap-2">
                  ...
                  {!role.includes(ROLES[2]) && (
                    <button
                      onClick={() => {
                        deleteHandler(selectedProduct._id);
                        setSelectedProduct(null);
                      }}
            ...
            <div className="modal-card bg-white w-full max-w-[560px] max-h-[90vh] overflow-hidden rounded-3xl shadow-2xl flex flex-col">
                  ...
                  <div>
                    ...
                    {bulkDiscountProduct && (
                      <p className="text-white/60 text-[12px] mt-0.5">
                        {bulkDiscountProduct.englishTitle}
                      </p>
                    )}
                  </div>
                  <button
              ...
              <div className="no-scroll overflow-y-auto flex-1 px-6 py-5">
                ...
                {discountRows.map((row, index) => (
                  <div
                    key={index}
                    className="grid grid-cols-[1fr_1fr_1fr_auto] gap-3 mb-3 items-center"
                  >
                    <input
                      type="number"
                      placeholder="e.g. 10"
                      value={row.quantity}
                      onChange={(e) =>
                        updateDiscountRow(index, "quantity", e.target.value)
                      }
                      className={inputCls}
                      min="1"
                    />
                    <input
                      ...
                      value={row.discount}
                      onChange={(e) =>
                        updateDiscountRow(index, "discount", e.target.value)
                      }
                      className={inputCls}
                      min="0"
                    ...
                    <select
                      value={row.type}
                      onChange={(e) =>
                        updateDiscountRow(index, "type", e.target.value)
                      }
                      className={`${inputCls} filter-select`}
                    >
