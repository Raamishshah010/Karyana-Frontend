...
const Supplier = () => {
        ...
        .test(
          ...
          (value) => {
            ...
            return !allPurchases.some(
              (p) =>
                p.accountNo?.trim() === value.trim() && p._id !== editingId,
            );
          },
        ),
    });
  ...
  useEffect(() => {
    ...
    if (filterStatus === "active")
      filtered = purchases.filter((p) => p.isActive);
  ...
  const statusToggleHandler = async (purchase) => {
    try {
      ...
      toast.success(
        `Supplier ${!purchase.isActive ? "activated" : "deactivated"}`,
      );
    } catch (err) {
  ...
  const handleSubmit = async (values, { resetForm, setSubmitting }) => {
    try {
      ...
      else payload.balance = Number(payload.balance);

      if (state.id) {
        await updatePurchase(state.id, payload);
        ...
        <div className="flex items-center justify-between mt-6 mb-5">
          <div>
            ...
            <p className="text-sm text-[#9CA3AF] mt-0.5">
              {filteredPurchases.length} suppliers found
            </p>
          </div>
          <button
            onClick={addHandler}
                  ...
                  <tr key={item._id || index} className="trow">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1">
                        <button
      onClick={() => navigate(`/Suppliers/Ledger/${item._id}`, { state: { supplier: item } })}
                ...
                <div className="relative flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-white/50 text-[11px] mt-1">
                      Only Company Name is required · all other fields are
                      optional
                    </p>
                  </div>
                  <button
                    ...
                    <div className="px-6 py-5 flex flex-col gap-3">
                      ...
                      <div>
                        ...
                        <div className="relative">
                          <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[13px] text-[#9CA3AF] font-semibold pointer-events-none select-none">
                            PKR
                          </span>
                          <input
                            name="balance"
